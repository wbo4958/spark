...
class SparkConnectBuildResourceProfileHandler(
    responseObserver: StreamObserver[proto.BuildResourceProfileResponse]) extends Logging {

  /**
   * transform the spark connect ResourceProfile to spark ResourceProfile
   * @param rp Spark connect ResourceProfile
   * @return the Spark ResourceProfile
   */
  private def transformResourceProfile(rp: proto.ResourceProfile):
    ...
    val ereqs = rp.getExecutorResourcesMap.asScala.map {
      case (name, res) => name -> new ExecutorResourceRequest(
        res.getResourceName, res.getAmount, res.getDiscoveryScript, res.getVendor)
    }.toMap
    val treqs = rp.getTaskResourcesMap.asScala.map {
      case (name, res) => name -> new TaskResourceRequest(res.getResourceName, res.getAmount)
    }.toMap
