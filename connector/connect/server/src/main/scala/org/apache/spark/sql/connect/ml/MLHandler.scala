  ...
  def transformMLRelation(
    ...
    mlRelationProto.getMlRelationTypeCase match {
      case proto.MlRelation.MlRelationTypeCase.MODEL_TRANSFORM =>
        ...
        val ids = modelTransformRelationProto.getModelRef.getId.split("\\.")
        assert(ids.length >= 1)
        val modelId = ids.head
        ...
        if (ids.length == 1) {
          ...
          // TODO try below invokeMethod
          val x = ids.slice(1, ids.length).foldLeft(model.asInstanceOf[Object]) {
            (obj, attribute) => invokeMethod(obj, attribute)
          }
          x.asInstanceOf[DataFrame]
        }
      ...
      //          None
      //        }
      //        algo.getModelSummaryAttr(copiedModel, modelSummaryAttr.getName,
      //        datasetOpt).right.get
